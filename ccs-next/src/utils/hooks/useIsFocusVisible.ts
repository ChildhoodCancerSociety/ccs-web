...
import { FocusEvent, MutableRefObject, Ref, useCallback, useRef } from "react";
 ...
 *
 * @param {Element} node element to watch
 * @returns {boolean} auto trigger on or off
 */
 ...
 *
 * @param {KeyboardEvent} event event to track if keyboard is active
 */
...
export default function useIsFocusVisible(): UseIsFocusVisibleResult {
   ...
   *
   * @returns {boolean} is blurred or not
   */
   ...
   * @returns {boolean} whether the node the event handler is attached to is focused
   */
